...
const handleBrokerRequest = async (req, res) => {
  ...
  if (!serviceUrl) {
    return res.status(503).json({
      ...
      message: "Revisa el archivo .env y asegúrate de tener la URL definida",
    });
  }
  ...
  try {
    ...
    if (error.code === "ECONNREFUSED" || error.code === "ENOTFOUND") {
      return res.status(503).json({
        success: false,
        service: matchedRoute.service,
        error: `El servicio '${matchedRoute.service}' no está disponible`,
        message: "El microservicio no está corriendo. Avisa al equipo correspondiente.",
        targetUrl,
      });
    }
    ...
    if (error.response) {
      return res.status(error.response.status).json({
        success: false,
        service: matchedRoute.service,
        error: "El microservicio respondió con un error",
